...
<body>
    ...
    <!-- نافذة تقرير الملفات -->
    <div class="file-report-modal" id="fileReportModal">
        <div class="file-report-content">
            <div class="file-report-header">
                <h3 class="file-report-title" data-ar="تقرير الملفات المرفوعة" data-en="Uploaded Files Report">تقرير الملفات المرفوعة</h3>
                <button class="close-btn" id="closeFileReportModal">&times;</button>
            </div>
            <div class="file-report-list" id="fileReportList">
                <!-- قائمة الملفات ستضاف هنا بالجافاسكريبت -->
                <div class="empty-message" data-ar="لا توجد ملفات مرفوعة بعد" data-en="No files uploaded yet">لا توجد ملفات مرفوعة بعد</div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const welcomeMessages = {
            ar: 'مرحباً! أنا هنا لمساعدتك في إنشاء النماذج الخاصة بك. هيا بنا نبدأ رحلة إنشاء النماذج الذكية معاً...',
            en: 'Hello! I\'m here to help you create your own models. Let\'s start the journey of creating smart models together...'
        };
        const botResponses = {
            ar: [
                'هذا سؤال ممتاز! اسمح لي بالتفكير في الإجابة...',
                'لدي الإجابة المناسبة لسؤالك...',
                'شكراً لسؤالك، هذا ما وجدته لك...',
                'إليك ما تعلمته عن هذا الموضوع...'
            ],
            en: [
                'This is an excellent question! Let me think about the answer...',
                'I have the right answer for your question...',
                'Thank you for your question, here\'s what I found for you...',
                'Here\'s what I learned about this topic...'
            ]
        };
        const fileResponses = {
            ar: [
                'شكراً لإرسال الملف. سأقوم بتحليله لك...',
                'لقد تلقيت ملفك وسأحاول تحليل محتواه...',
                'هذا ملف مثير للاهتمام! دعني أفحصه وأقدم لك المعلومات...'
            ],
            en: [
                'Thank you for sending the file. I will analyze it for you...',
                'I have received your file and will try to analyze its content...',
                'This is an interesting file! Let me examine it and provide you with information...'
            ]
        };
        ...
        const fileReportModal = document.getElementById('fileReportModal');
        const closeFileReportModal = document.getElementById('closeFileReportModal');
        const fileReportList = document.getElementById('fileReportList');
        ...
        function addFile(fileData, caption, fileType, isUser) {
            ...
            if (isUser) {
                ...
                // إرسال رسالة تجهيز المدن بعد رفع الفيديو
                if (fileType === 'video' || fileType === 'video-url') {
                    setTimeout(() => {
                        addMessage(currentLanguage === 'ar' ? 'جاري تجهيز المدن الخاصة بك...' : 'Preparing your cities...', false);
                    }, 2000);
                }
            }
        }
        ...
        // تحديث تقرير الملفات
        function updateFileReport() {
            if (uploadedFiles.length === 0) {
                fileReportList.innerHTML = `<div class="empty-message" data-ar="لا توجد ملفات مرفوعة بعد" data-en="No files uploaded yet">${currentLanguage === 'ar' ? 'لا توجد ملفات مرفوعة بعد' : 'No files uploaded yet'}</div>`;
                return;
            }
            
            fileReportList.innerHTML = '';
            
            uploadedFiles.forEach((file, index) => {
                const fileItem = document.createElement('div');
                fileItem.className = 'file-report-item';
                fileItem.dataset.id = file.id;
                
                let fileIcon;
                switch(file.type) {
                    case 'image': fileIcon = 'fa-image'; break;
                    case 'video': fileIcon = 'fa-video'; break;
                    case 'pdf': fileIcon = 'fa-file-pdf'; break;
                    case 'video-url': fileIcon = 'fa-link'; break;
                    default: fileIcon = 'fa-file';
                }
                
                const fileDate = file.date.toLocaleString();
                const fileSize = file.type === 'video-url' ? 'URL' : 'N/A';
                
                fileItem.innerHTML = `
                    <div class="file-info">
                        <i class="fas ${fileIcon} file-icon"></i>
                        <div class="file-details">
                            <div class="file-name">${file.caption || (currentLanguage === 'ar' ? 'ملف بدون عنوان' : 'Untitled file')}</div>
                            <div class="file-size">${fileDate} • ${fileSize}</div>
                        </div>
                    </div>
                    <div class="file-actions">
                        <button class="file-action-btn view-file-btn" title="${currentLanguage === 'ar' ? 'عرض' : 'View'}">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button class="file-action-btn delete-file-btn" title="${currentLanguage === 'ar' ? 'حذف' : 'Delete'}">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                `;
                
                fileReportList.appendChild(fileItem);
            });
            
            // إضافة أحداث للأزرار
            document.querySelectorAll('.view-file-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    const fileId = parseInt(this.closest('.file-report-item').dataset.id);
                    const file = uploadedFiles.find(f => f.id === fileId);
                    if (file) {
                        if (file.type === 'video-url') {
                            window.open(file.data, '_blank');
                        } else {
                            // يمكنك هنا إضافة عرض الملف في نافذة منبثقة
                            alert(currentLanguage === 'ar' ? 'عرض الملف: ' + (file.caption || 'بدون عنوان') : 'Viewing file: ' + (file.caption || 'Untitled'));
                        }
                    }
                });
            });
        ...
        // فتح نافذة تقرير الملفات
        function openFileReportModal() {
            fileReportModal.style.display = 'flex';
            updateFileReport();
        }
        ...
        function closeModal() {
            ...
            fileReportModal.style.display = 'none';
            resetForms();
        }
        ...
        function handleImageUpload() {
            ...
            if (fileInput.files.length > 0) {
                ...
                reader.onload = function(e) {
                    ...
                    // محاكاة رد الذكاء الاصطناعي على الصورة
                    setTimeout(() => {
                        const responses = fileResponses[currentLanguage];
                        const randomResponse = responses[Math.floor(Math.random() * responses.length)];
                        addMessage(randomResponse, false);
                    }, 1500);
                };
        ...
        function handleVideoUpload() {
            ...
            if (fileInput.files.length > 0) {
                ...
                reader.onload = function(e) {
                    ...
                    // محاكاة رد الذكاء الاصطناعي على الفيديو
                    setTimeout(() => {
                        const responses = fileResponses[currentLanguage];
                        const randomResponse = responses[Math.floor(Math.random() * responses.length)];
                        addMessage(randomResponse, false);
        ...
        function handlePdfUpload() {
            ...
            if (fileInput.files.length > 0) {
                ...
                reader.onload = function(e) {
                    ...
                    // محاكاة رد الذكاء الاصطناعي على ملف PDF
                    setTimeout(() => {
                        const responses = fileResponses[currentLanguage];
                        const randomResponse = responses[Math.floor(Math.random() * responses.length)];
                        addMessage(randomResponse, false);
        ...
        function handleVideoUrl() {
            ...
            if (url) {
                ...
                // محاكاة رد الذكاء الاصطناعي على رابط الفيديو
                setTimeout(() => {
                    const responses = fileResponses[currentLanguage];
                    const randomResponse = responses[Math.floor(Math.random() * responses.length)];
                    addMessage(randomResponse, false);
        ...
        function handleDirectFileUpload(event) {
            ...
            reader.onload = function(e) {
                ...
                // محاكاة رد الذكاء الاصطناعي على الملف
                setTimeout(() => {
                    const responses = fileResponses[currentLanguage];
                    const randomResponse = responses[Math.floor(Math.random() * responses.length)];
                    addMessage(randomResponse, false);
                ...
                // إرسال رسالة تجهيز المدن إذا كان الملف فيديو
                if (type === 'video') {
                    setTimeout(() => {
                        addMessage(currentLanguage === 'ar' ? 'جاري تجهيز المدن الخاصة بك...' : 'Preparing your cities...', false);
                    }, 2000);
        ...
        closeFileReportModal.addEventListener('click', closeModal);
        ...
        window.addEventListener('click', (e) => {
            if (e.target === uploadModal || e.target === fileReportModal) {
                closeModal();
            }
        });
