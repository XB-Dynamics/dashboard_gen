  ...
  exams.forEach(exam => {
    ...
    card.innerHTML = `
      ...
      <div class="model-actions">
        ...
        <a href="create_test.php?index=${exam.index_name}" 
   class="model-btn primary-btn"
   data-num="${exam.num_questions}">
   <i class="fas fa-door-open"></i> دخول إلى الامتحان
    ...
    // عند الضغط على "دخول إلى الامتحان"
document.addEventListener("click", (e) => {
  if (e.target.closest(".model-btn.primary-btn")) {
    const btn = e.target.closest(".model-btn.primary-btn");
    const indexName = btn.getAttribute("href").split("index=")[1];
    const quiz = allExams.find(q => q.index_name === indexName);
